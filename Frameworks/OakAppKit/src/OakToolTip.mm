...
- (void)showAtLocation:(NSPoint)aPoint forScreen:(NSScreen*)aScreen
{
	...
	CGFloat const maxWidth = std::min<CGFloat>(NSWidth(r), 800);
	NSSize contentSize = [field.cell cellSizeForBounds:NSMakeRect(0, 0, maxWidth, CGFLOAT_MAX)];
	contentSize.width  = ceil(contentSize.width) + 6;
	contentSize.height = ceil(contentSize.height) + 4;
	NSRect frame = [self frameRectForContentRect:{ NSZeroPoint, contentSize }];
